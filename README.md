...
The current UI is responsive across mobile, tablet, and desktop layouts and includes mobile sheets for navigation/dropdowns, theme switching, grid/table collection views, and progressive filtering on the library/search screens.
...
The script auto-detects your package manager (bun preferred, npm as fallback), builds the frontend, and checks for PHP before starting.
...
The app now has two different search/refinement behaviors on purpose:

- **Server search** on `/search/`
  Submitting the main "Find happy thoughts" field sends `q` to the backend and returns a result set from the database.
- **Client-side filtering** on the results page
  The "Filter the current results" field and category dropdown refine only the thoughts already loaded on the page. They do not make another request.

This means the intended flow is:

1. run a database search to get a useful result set
2. narrow that result set locally with text filtering and category filtering

The two inputs are intentionally separate. Clearing the local filter should not restore or overwrite the server query.
...
- Search is hybrid: PHP performs database-backed searching, while Svelte handles client-side refinement and presentation state.
...
- If search results look unexpected, check whether you are looking at the server query, the local filter, or both at the same time.

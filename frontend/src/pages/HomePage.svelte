...
<section class="border-b border-mist">
  <div class="mx-auto max-w-6xl px-6 py-16 lg:px-8 lg:py-24">
    <div class="grid items-center gap-12 lg:grid-cols-2 lg:gap-16">
      <div>
        <h1
          class="mb-6 font-display text-3xl text-ink sm:text-4xl lg:text-5xl xl:text-6xl"
        >
          A collection of happy thoughts for brighter days.
        </h1>
        <p class="mb-8 max-w-lg leading-relaxed text-stone">
          This content management system demonstrates PHP, SQLite, Svelte, and
          modern web development. Create, browse, and search through moments of
          joy.
        </p>
      ...
      <div class="relative">
        ...
        />
        <div
          class="absolute -bottom-4 -left-4 hidden bg-canvas-elevated p-4 shadow-lg lg:block"
        >
          <SmilePlus size={20} class="mb-2 text-coral" />
...
<section class="border-b border-mist bg-canvas-elevated">
  <div class="mx-auto max-w-6xl px-6 lg:px-8">
    <div
      class="grid grid-cols-2 gap-px bg-mist md:grid-cols-4 md:gap-0 md:divide-x md:divide-mist md:bg-transparent"
    >
      <div
        class="bg-canvas-elevated px-4 py-7 text-center md:bg-transparent md:py-8"
      >
        <FileText size={20} class="mx-auto mb-2 text-coral" />
        <p class="font-display text-3xl text-ink">{stats.total}</p>
        <p class="mt-1 text-sm text-stone">Total Records</p>
      </div>
      <div
        class="bg-canvas-elevated px-4 py-7 text-center md:bg-transparent md:py-8"
      >
        <Tag size={20} class="mx-auto mb-2 text-coral" />
        <p class="font-display text-3xl text-ink">{stats.categories}</p>
        <p class="mt-1 text-sm text-stone">Categories</p>
      </div>
      <div
        class="bg-canvas-elevated px-4 py-7 text-center md:bg-transparent md:py-8"
      >
        <Star size={20} class="mx-auto mb-2 text-coral" />
        <p class="font-display text-3xl text-ink">{stats.highMood}</p>
        <p class="mt-1 text-sm text-stone">High Mood (4-5★)</p>
      </div>
      <div
        class="bg-canvas-elevated px-4 py-7 text-center md:bg-transparent md:py-8"
      >
        <Image size={20} class="mx-auto mb-2 text-coral" />
        <p class="font-display text-3xl text-ink">{stats.withImages}</p>
        <p class="mt-1 text-sm text-stone">With Images</p>
      </div>
    </div>
  </div>
</section>
...
<Section>
  <div>
    <div
      class="mb-8 flex flex-col items-start gap-3 sm:flex-row sm:items-end sm:justify-between sm:gap-4"
    >
      <div>
        <DisplayHeading
      ...
      <a
        ...
        class="editorial-link inline-flex items-center gap-2 text-sm"
      >
        View all thoughts
        <ArrowRight size={16} />
      </a>

...
export function getMinimumVisibleHeight(viewportHeight: number): number {
  return Math.max(
    viewportHeight * SHEET_MIN_VIEWPORT_RATIO,
    SHEET_MIN_ABSOLUTE_HEIGHT_PX,
  );
}
...
export function getMaximumSheetHeight(viewportHeight: number): number {
  return Math.max(
    viewportHeight - getExpandedTopOffset(),
    getMinimumVisibleHeight(viewportHeight),
  );
}
...
export function getScrollContentHeight(viewportHeight: number): number {
  return Math.max(
    getMaximumSheetHeight(viewportHeight) - SHEET_HEADER_OFFSET_PX,
    220,
  );
}

export function getRestingTranslateY(

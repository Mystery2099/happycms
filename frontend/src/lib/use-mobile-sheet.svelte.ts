...
function getDragResistance(variant: SheetVariant) {
	return variant === 'user-menu' ? USER_MENU_DRAG_RESISTANCE : DROPDOWN_DRAG_RESISTANCE;
}
...
export function createMobileSheetManager(callbacks: MobileSheetCallbacks) {
	...
	function updateSheetStyles() {
		...
		if ((!isOpen && !isMobileSheetVisible) || callbacks.getViewportWidth() >= 768) {
	...
	function handleDragStart(event: PointerEvent, currentOffset: number, setPointerCapture: (id: number) => void): boolean {
	...
	function handleDragMove(event: PointerEvent, options: { onStartDrag?: () => void }) {
		if (event.pointerId !== dragState.activePointerId) return null;

		dragState.dragDeltaY = event.clientY - dragState.dragStartY;
		if (!dragState.isDragging && Math.abs(dragState.dragDeltaY) < SHEET_SNAP_THRESHOLD_PX) return null;
		...
		let nextOffset = Math.max(0, dragState.dragStartOffset + dragState.dragDeltaY);
	...
	function handleDragEnd(event: PointerEvent, releasePointerCapture: (id: number) => void, focusTrigger: () => void) {
		if (event.pointerId !== dragState.activePointerId) return null;
		...
		if (variant === 'user-menu') {
			shouldClose = currentTranslateY > closeThreshold && dragState.dragDeltaY > 0;
		} else {
			const viewportHeight = callbacks.getViewportHeight();
			const collapsedOffset = getCollapsedOffset(viewportHeight);
			const absoluteCloseThreshold = Math.max(collapsedOffset + closeThreshold, viewportHeight * 0.72);
			shouldClose = dragState.dragDeltaY > 0 && currentTranslateY > absoluteCloseThreshold;
		}
		...
		return {
			...
			finishDrag: () => {
				if (variant === 'dropdown') {
					const viewportHeight = callbacks.getViewportHeight();
					const collapsedOffset = getCollapsedOffset(viewportHeight);
					const wantsExpand = dragState.dragDeltaY < -SHEET_SNAP_THRESHOLD_PX || currentTranslateY < collapsedOffset * 0.5;
					const wantsCollapse = dragState.dragDeltaY > SHEET_SNAP_THRESHOLD_PX || currentTranslateY >= collapsedOffset * 0.5;
					...
					isDragging = false;
					currentTranslateY = Math.max(0, getRestingTranslateY(viewportHeight, isExpanded));
				} else {
					isDragging = false;
					if (shouldClose) {
						closeSheet(focusTrigger);
	...
	return {
		...
		set isExpanded(v: boolean) { isExpanded = v; },
		get isDragging() { return isDragging; },
		set isDragging(v: boolean) { isDragging = v; },
		get isMobileSheetVisible() { return isMobileSheetVisible; },
		set isMobileSheetVisible(v: boolean) { isMobileSheetVisible = v; },
		get isOverlayVisible() { return isOverlayVisible; },
		set isOverlayVisible(v: boolean) { isOverlayVisible = v; },
		get isAnimatingIntro() { return isAnimatingIntro; },
		set isAnimatingIntro(v: boolean) { isAnimatingIntro = v; },
		get isClosingSheet() { return isClosingSheet; },
		set isClosingSheet(v: boolean) { isClosingSheet = v; },
		get currentTranslateY() { return currentTranslateY; },
		set currentTranslateY(v: number) { currentTranslateY = v; },

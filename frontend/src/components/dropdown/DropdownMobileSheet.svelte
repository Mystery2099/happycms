...
>
  <div class={mobileSheetHeaderClass}>
    ...
    <div
      class="border-stone-200/70 flex items-center justify-between border-b px-5 pb-3 pt-2 dark:border-slate-800"
    >
      <div>
        <p
        ...
        {#if shouldHintMoreContent}
          <p
            class="mt-1 flex items-center gap-1.5 text-xs font-medium text-coral dark:text-rose-300"
          >
            <span
              class="bg-coral/10 inline-flex h-5 w-5 items-center justify-center rounded-full dark:bg-rose-300/10"
            >
  ...
  <div
    class="dropdown-sheet-scroll relative px-4 pb-[max(1rem,env(safe-area-inset-bottom))]"
  >
    <div
    ...
    >
      {#each options as option (option.value)}
        ...
        <button
          type="button"
          class="flex min-h-14 w-full items-center justify-between gap-3 rounded-md border px-4 py-3 text-left transition-colors {value ===
          option.value
            ? 'bg-coral/10 border-coral text-ink shadow-sm dark:border-rose-400 dark:bg-slate-700 dark:text-slate-50'
            : 'border-stone-200/80 hover:border-stone-300 hover:bg-stone-50 bg-white/90 text-stone hover:text-ink dark:border-slate-700 dark:bg-slate-800/90 dark:text-slate-300 dark:hover:border-slate-500 dark:hover:bg-slate-700/95 dark:hover:text-slate-50'}"
        ...
        >
          <span class="flex min-w-0 items-center gap-3">
            {#if Icon}
              <span
                class="bg-stone-100 flex h-10 w-10 items-center justify-center rounded-md text-stone dark:bg-slate-700 dark:text-slate-200"
              >
                <Icon size={18} />
              </span>
            {/if}
          ...
          {#if value === option.value}
            <span
              class="flex h-8 w-8 items-center justify-center rounded-full bg-coral text-white dark:bg-rose-400 dark:text-slate-950"
            >
              <Check size={16} />
            </span>
          {/if}
    ...
    {#if shouldHintMoreContent}
      <div
        class="via-canvas/92 pointer-events-none absolute inset-x-4 bottom-0 h-24 rounded-t-xl bg-gradient-to-t from-canvas to-transparent dark:from-slate-900 dark:via-slate-900/90"
        aria-hidden="true"
      ></div>
    {/if}
  </div>

...
<div
	class={['relative rounded-lg p-0.5 bg-mist/50 dark:bg-slate-800/50', groupClass]}
	role="group"
...
>
	<div class="absolute inset-0 p-0.5" aria-hidden="true">
		<div
			class="h-full rounded-md bg-white shadow-sm ring-2 ring-coral ring-offset-1 dark:bg-slate-700 dark:ring-offset-slate-800 transition-[transform,width] duration-200 ease-enter motion-reduce:transition-none"
			style={activeStyle}
		></div>
	</div>
	...
	>
		{#each options as option (option.value)}
			...
			<button
				type="button"
				class={[
					'min-w-0 z-10 flex items-center justify-center gap-2 rounded-md text-sm font-medium text-stone transition-colors duration-200 active:scale-[0.97] motion-reduce:transition-none',
					buttonClass,
					value === option.value

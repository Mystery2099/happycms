<script lang="ts">
  ...
  const triggerClass = $derived(
    isMobileVariant
      ...
      : getDesktopSurfaceTriggerClass({
          extraClass: "dropdown-trigger-inline",
          isOpen,
        }),
  );
</script>
...
>
  <span
    class={isMobileVariant
      ? "flex flex-col items-center gap-1"
      : "flex min-w-0 items-center gap-3"}
  >
    <span
      class="bg-coral/10 flex h-8 w-8 items-center justify-center rounded-full text-coral dark:bg-rose-400/15 dark:text-rose-300"
    >
      <User size={16} />
    </span>
    {#if isMobileVariant}
    ...
    {:else}
      <span class="hidden min-w-0 lg:block">
        <span
          class="block max-w-[120px] truncate text-sm font-medium text-ink dark:text-slate-100"
        >
          {isLoggedIn ? userName || "User" : "Menu"}
        </span>
        <span
          class="block max-w-[140px] truncate text-xs text-stone dark:text-slate-400"
        >
          {isLoggedIn
            ? userEmail || "Preferences and account"
            : "Theme and sign in"}
        </span>
      </span>
    {/if}

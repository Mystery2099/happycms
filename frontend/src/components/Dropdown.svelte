<script lang="ts">
  ...
  function handleDesktopOptionKeydown(event: KeyboardEvent, index: number) {
    ...
    if (event.key === "End") {
      event.preventDefault();
      highlightedIndex = jumpHighlight(
        options,
        options.length - 1,
        optionElements,
      );
  ...
  function handleHandlePointerDown(event: PointerEvent) {
    ...
    const offset =
      sheet.currentTranslateY ||
      getRestingTranslateY(viewportHeight, sheet.isExpanded);
  ...
  function toggleSheetExpansion() {
    ...
    const viewportHeight = innerHeight.current ?? 900;
    sheet.currentTranslateY = Math.max(
      0,
      getRestingTranslateY(viewportHeight, sheet.isExpanded),
    );
    sheet.updateSheetStyles();
...
<div bind:this={rootElement} class="relative {className}" data-dropdown-root>
  ...
  {#if isOpen && !isMobile}
    <div
      ...
      class="origin-top animate-dropdown-enter motion-reduce:animate-none"
      data-dropdown-open
    >
      <DropdownDesktopList
        {options}
